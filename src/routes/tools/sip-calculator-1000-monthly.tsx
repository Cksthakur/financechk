...
export const Route = createFileRoute("/tools/sip-calculator-1000-monthly")({
  head: () =>
    generateSeoMeta({
      title: `SIP Calculator ${AMOUNT_LABEL} Monthly - ${TENURE_YEARS} Year Returns`,
      description: `Free SIP calculator for ${AMOUNT_LABEL} monthly investment. Calculate returns over ${TENURE_YEARS} years with ${RETURN_RATE}% expected return. See how small investments grow to big corpus.`,
...
const faqs = [
  {
    ...
    answer: `At ${RETURN_RATE}% expected return, your ${AMOUNT_LABEL} monthly SIP for ${TENURE_YEARS} years will grow to approximately ₹2.3 lakh. You would have invested ₹1.2 lakh total, earning ₹1.1 lakh in returns. That's a 92% return on your investment!`,
  },
  {
    question: `Is ${AMOUNT_LABEL} monthly SIP enough for financial goals?`,
    answer: `${AMOUNT_LABEL} monthly SIP can help build significant wealth over time. For a ${TENURE_YEARS}-year goal at ${RETURN_RATE}%, it creates ₹2.3 lakh. For 20 years, it grows to ₹10 lakh. Increase your SIP amount as your income grows (step-up SIP).`,
  },
  {
    question: "Which mutual funds are best for ₹1,000 SIP?",
    answer:
      "For beginners, index funds like Nifty 50 or Sensex index funds are ideal as they have very low costs (0.1% expense ratio) and match market returns. For higher risk tolerance, consider Flexicap or Smallcap funds for potentially higher returns.",
  },
]
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-5xl space-y-6 px-0 py-8">
        ...
        <div className="rounded-2xl bg-muted/50 p-6">
          <h3 className="mb-4 text-lg font-semibold">Year-by-Year Breakdown</h3>
                  ...
                  .map((row) => (
                    <tr key={row.year} className="border-b">
                      ...
                      <td className="text-right">
                        {formatCompactCurrency(row.invested)}
                      </td>
                      <td className="text-right text-green-600">
                        +{formatCompactCurrency(row.returns)}
                      </td>
                      <td className="text-right font-medium">
                        {formatCompactCurrency(row.corpus)}
                      </td>
                    </tr>
                  ))}
              </tbody>
